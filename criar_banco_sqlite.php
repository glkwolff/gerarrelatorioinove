<?php
// cria o bd pra testes
try {
    // 1. Conexão com o SQLite
    $pdo = new PDO('sqlite:meu_banco_de_dados.sqlite');
    
    // Habilita a exibição de erros do PDO
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // !! ADICIONADO !! - Apaga a tabela 'alunos' se ela já existir
    $pdo->exec("DROP TABLE IF EXISTS alunos");

    // 2. SQL para criar a tabela 'alunos' com a estrutura correta
    $sql_create_table = "
    CREATE TABLE alunos (
      id_aluno INTEGER PRIMARY KEY AUTOINCREMENT,
      nome TEXT NOT NULL,
      email TEXT NOT NULL,
      status TEXT NOT NULL,
      valor NUMERIC
    )";

    // Executa a criação da tabela
    $pdo->exec($sql_create_table);
   /* 
    // 3. SQL para inserir os dados de exemplo
    $stmt = $pdo->query("SELECT COUNT(*) FROM alunos");
    if ($stmt->fetchColumn() == 0) {
        $sql_insert_data = "
        INSERT INTO alunos (nome, email, status, valor) VALUES
        ('ANA CAROLINA PEREIRA', 'ana.carolina@email.com', 'Ativo',103),
        ('BRUNO DIAS DE ALMEIDA', 'bruno.dias@email.com', 'Ativo',48),
        ('CARLOS ANDRADE LIMA', 'carlos.lima@email.com', 'Inativo',35),
        ('DANIELA MARTINS ROCHA', 'daniela.rocha@email.com', 'Ativo',20),
        ('EDUARDO FERNANDES', 'eduardo.f@email.com', 'Ativo',15);
        ";
        // Executa a inserção dos dados
        $pdo->exec($sql_insert_data);
        echo "Banco de dados e tabela 'alunos' recriados com sucesso! 5 registros inseridos.";
    } else {
        echo "A tabela 'alunos' já contém dados. Nenhuma ação foi necessária.";
    }
*/
    echo "<p>Tabela 'alunos' criada com sucesso.</p>";
} catch (PDOException $e) {
    // Exibe o erro caso algo dê errado
    die("Erro ao conectar ou configurar o banco de dados SQLite: " . $e->getMessage());
}